...
use std::{io, io::Error, sync::Arc, sync::Mutex, sync::mpsc, thread};
...
fn main() {
    ...
    let handle_tread = thread::Builder::new()
        .name("handle".into())
        .spawn(move || main_handle_events(event_sd))
        .unwrap();

    let draw_thread = thread::Builder::new()
        .name("draw".into())
        .spawn({
            let draw_app: Arc<Mutex<App>> = Arc::clone(&mutex_tread_app);
            move || draw_app.lock().unwrap().run(ratatui::init(), event_rc)
        })
        .unwrap();
