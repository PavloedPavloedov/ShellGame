pub mod tab_logic {
    ...
    impl TabManager {
        ...
        pub fn add_tab(&mut self, new_tab_uuid: Uuid, tab_name: String) -> io::Result<Uuid> {
        ...
        pub fn remove_tab_from_list(&mut self, tab_uuid: Uuid) -> io::Result<Uuid> {
            match self.tab_hashmap.remove_entry(&tab_uuid) {
                ...
                None => Err(io::Error::new(io::ErrorKind::NotFound, "Вкладка не найдена")),
            }
        }
        ...
        pub fn remove_current_tab(&mut self) -> io::Result<Uuid> {
            match self.current_tab_uuid {
                ...
                None => Err(io::Error::new(io::ErrorKind::NotFound, "Пустой список вкладок")),
            }
        }
